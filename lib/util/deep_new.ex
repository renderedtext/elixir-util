defmodule Util.Proto do
  ...
  def deep_new(struct, args, opts \\ []), do: deep_new(args, struct, "", opts)

  defp deep_new(args, struct, name, opts) 
  ...
  defp deep_new(arg, _struct = :bool, _name, _opts) when is_boolean(arg), do: arg
  defp deep_new(arg, _struct = :bool, name, _opts),
    do: raise "Field: '#{name}': Expected boolean argument, got '#{inspect arg}'"

  defp deep_new(arg, _struct = :string, _name, _opts) when is_binary(arg), do: arg
  defp deep_new(arg, _struct = :string, name, _opts),
    do: raise "Field: '#{name}': Expected string argument, got '#{inspect arg}'"

  defp deep_new(arg, _struct = :enum, _name, _opts) when is_integer(arg), do: arg
  defp deep_new(arg, _struct = :enum, name, _opts),
    do: raise "Field: '#{name}': Expected integer or atom argument, got '#{inspect arg}'"

  defp deep_new(arg, struct, _name, _opts)
    when struct in [:int32, :int64, :uint32, :uint64, :sint32, :sint64] and
      is_integer(arg), do: arg
  defp deep_new(arg, struct, name, _opts)
